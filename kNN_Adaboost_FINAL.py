...
if __name__=="__main__":
    ...
    for i in range(1,10): # j'ai 2 bootstraps
        
        df_copy=train_data.sample(frac=0.3,replace=True).reset_index(drop=False)   # je prend tout mon échantillon train, mais c'est echantillon AVEC REMISE
...
# #             # nombre de voisins associés à l'erreur minimum
    ...
    for i in range(len(y_predictions)):
        ...
        y_predictions['max frequency']=scipy.stats.mode(y_predictions.values, 1)[0]
    
    y_predicts_final=y_predictions['max frequency'].tolist()
    
    # y_actual = test_data.iloc[:,:-1]
    
    incorrect=0
    for i in range(len(y_boot_list)):
        if y_boot_list[i] != y_predicts_final[i]:
            incorrect += 1
    	
    print('Error : ',incorrect / float(len(y_boot_list)) * 100.0)
